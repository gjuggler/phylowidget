...
function loadYahoo(callback)
{
	/*
	 * Load up the YUI loader, and import some required YUI components.
	 */
	var loader = new YAHOO.util.YUILoader({
	    require: ["event","dragdrop","container"
	    ],
	    loadOptional:false,
	    onSuccess: callback
	    //onFailure: function(msg,xhrobj) {alert(""+msg.toString()+" "+xhrobj);}
	  });
	  loader.insert();
}
...
/*
 * (USED TO BE) APPLETLOADER.JS
 * 
 * Built on ${TODAY} for PhyloWidget
 */
...
function loadApplet(div_id,name,propsRef)
{
	applet(div_id,name,propsRef);
}

function loadAppletPanel(id,propsRef,callback)
{
	var aoCall = function() {
		applet('applet_holder_'+id,id,propsRef);
		if (callback)
			callback();
	};
	loadSketchPanel(id,aoCall);
}

function loadAppletObject(div_id,id,propsRef,callback)
{
	appletObject(div_id,id,propsRef,callback);
}

function loadAppletObjectPanel(id,propsRef,callback)
{
	var aoCall = function() {
		appletObject('applet_holder_'+id,id,propsRef);
		callback();
	};
	loadSketchPanel(id,aoCall);
}
...
function loadSketchPanel(id,callback)
{
	// TODO: Support resizing panels by using some premade YUI code:
	//  http://developer.yahoo.com/yui/examples/container/panel-resize.html
  var myF = function ()
  {
	  // Create the panel.
	  var cfg = {visible:true,constraintoviewport:true,resizable:true};
	  var panel2 = new YAHOO.widget.Panel(id+"_p", cfg);
	  panel2.setHeader("${project.name}");
	  panel2.setBody("<div id='applet_holder_"+id+"' style='width:${applet.width}px;'></div>");
//	  panel2.setFooter("Created with <a href='http://www.processing.org/'>Processing</a> and <a href='http://www.andrewberman.org/'>P5Barebones</a>");
	  panel2.hideEvent.subscribe(function() {
	  	setTimeout(function() {stopApplet(id);panel2.destroy();},10);
	  });
	  AO[id] = panel2;
	  panel2.beforeRenderEvent.subscribe(callback);
...
function applet(container_id,id,propsRef)
{
	...
	'<applet id="'+id+'" name="'+id+'" code="${internal.launcherClass}" ' +
	 'codebase="'+codebase+'${lib}" archive="${internal.allJars}" style="width:${applet.width}px;height:${applet.height}px;" mayscript="true">' +
...
function appletObject(container_id,id,propsRef,callback)
{
	var myF = function() {
	  var ao = new AppletObject(
		...
		[], // additional params
		AppletObjects.TAG_APPLET, // Type of insertion -- either AppletObjects.TAG_APPLEt or AppletObject.TAG_OBJECT 
		id
	  );
	  ao.addParams(['boxbgcolor','#FFFFFF']);
	  ao.addParams(['image','loading.gif'])
	...
	loadScriptWithCallback(codebase+"scripts/appletobject.js",myF);
}
...
function loadToolbox(dest_id)
{
	...
	"<h1>Tree:</h1>" +
	"<div class='indent'>" + getTextFieldHTML(treeInputID,"updateJavaTree()","Clickaway to update the PhyloWidget tree.") + "</div>"+
	"<h1>Clipboard:</h1>" +
	"<div class='indent'>"+ getTextFieldHTML(clipInputID,"updateJavaClip()","Clickaway to update the PhyloWidget clipboard.") + "</div>" +
	"<h1>Node Info:</h1>" +
	"<div class='indent' id='"+nodeInfoID+"' style='height:150px;'></div>" +
	"<div id='nodeInfoFooter' style=''></div>"
	;
	PW.toolbox = getObject(dest_id);
}

function getTextFieldHTML(id,fnToCall,msg)
{
...
function loadPhyloWidget(callback)
{
	var id = 'PhyloWidget';
	var myF = function() {
		PW.PhyloWidget = AO[id];
		PW.PhyloWidget.cfg.applyConfig({close:false,x:50});
		if (callback)
			callback();
		//PW.PhyloWidget.render();
	};
	var props = getQueryParameters();
	loadAppletObjectPanel(id,props,myF);
}
/*
function loadToolbox(callback)
{
	var myF = function() {
		// Load the panel.
	  	var toolbox = new YAHOO.widget.Panel("nodeInfo", {width:"300px",visible:true, constraintoviewport:true,resizable:true,close:true} );
	  	toolbox.setHeader("${project.name} toolbox");
	  	toolbox.setBody("");
	  	toolbox.setFooter("<div id='nodeInfoFooter'></div>")
	  	toolbox.hideEvent.subscribe(function() {
	  		//toolbox.destroy();
	  		PW.toolbox = null;
	  	});
	  	var cb = function() {
	  	...
	  	var loadKeys = function() {
			var skeys = YAHOO.util.KeyListener.KEY;
	  		var k1 = new YAHOO.util.KeyListener(treeInputID,
	  							{keys:[skeys.ENTER]},
	  							updateJavaTree
	  							);
	  		var k2 = new YAHOO.util.KeyListener(clipInputID,
	  							{keys:[skeys.ENTER]},
	  							updateJavaClip
	  							);
	  		k1.enable();
	  	...
	  	toolbox.beforeRenderEvent.subscribe(cb);
	  	toolbox.showEvent.subscribe(loadKeys);
...
/*
 * INTERNAL FUNCTIONS -- DON'T WORRY ABOUT THESE!!
 */
...
function connectToJava()
{
	...
	{
		updateFooter("Connecting to Java -- please be patient <span id='ellipsisSpan'></span>");
		startEllipsis();
		getObject(treeInputID).disabled = true;
		getObject(clipInputID).disabled = true;
		connecting = true;
		setTimeout(connectToJava,50);
		return;
	}
	if (!connected)
	{
		var f = document.applets;
		var s = '';
		for (var i=0; i < f.length; i++)
		{
			if (document.applets[i].name == 'PhyloWidget')
			{
				var result = document.applets[i].isActive();
				if (!result)
				{
					setTimeout(connectToJava,500);
					return;
				}
				// If we've made it here, then Java must be doing something right.
				getObject(treeInputID).disabled = false;
				getObject(clipInputID).disabled = false;
				updateFooter("Connected to Java! Use the text inputs to alter the tree.");
			  	stopEllipsis();
			  	connected = true;
			  	return;
			}
...
/*
 * Some cute ellipsis stuff...
 */
var ellipsis = ['...','&nbsp;...','&nbsp;&nbsp;..','&nbsp;&nbsp;&nbsp;.', '','.','..'];
var runEllipsis = false;
function animateEllipsis(el, count) {
	el.innerHTML = ellipsis[count%ellipsis.length];
	if(runEllipsis == true) {
...
function startEllipsis() {
	runEllipsis = true;
	animateEllipsis(getObject('ellipsisSpan'), 0);
}
